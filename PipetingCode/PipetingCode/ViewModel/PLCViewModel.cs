...
namespace PipettingCode.ViewModel
{
    public class PLCViewModel : NotifiactionObject
    {
        ...
        private void Initial()
        {
            try
            {
                MyPort.PortName = MySettingWindow.COM;                       // 默认端口就是COM3
                MyPort.BaudRate = Convert.ToInt32(115200);      // 波特率
                MyPort.Parity = Parity.Odd;                     // 奇校验
                MyPort.DataBits = Convert.ToInt16(8);           // 8位数据位
                MyPort.StopBits = StopBits.One;
                MyPort.Open();
                MyPort.ReadTimeout = 800;                      // 0.8秒读超时
                MyPort.WriteTimeout = 800;                     // 0.8秒写超时
        ...
        private int PLC_RCS(string address)
        {
            try
            {
                ...
                lock (_locker)
                {
                    ...
                    string? receive = MyPort.ReadTo("\x0d");
                    if (receive[3] == '$')
                    {
        ...
        private int[] PLC_RDArray(string beginAddress, int size)
        {
            ...
            try
            {
                ...
                {
                    address = "D" + beginAddress.Substring(2, beginAddress.Length - 2);
                    endAddress = string.Format("{0:D5}", Convert.ToInt32(address.Substring(1, address.Length - 1)) + size - 1);
                }
                else if (beginAddress.StartsWith("DDT"))
                {
                    address = "D" + beginAddress.Substring(3, beginAddress.Length - 3);
                    endAddress = ((Convert.ToInt32(address.Substring(1, address.Length - 1))) + 2 * size - 1).ToString();
                }
                string command = "%01#RD" + address + endAddress + "**\r";
